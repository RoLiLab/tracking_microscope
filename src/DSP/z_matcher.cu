...
__inline__ __device__ floatint findmax4(float4 val, int stride) {
	float max = fmaxf(fmaxf(fmaxf(val.x, val.y), val.z), val.w);
	floatint r0 = { max, 0 };
	floatint r1 = { max, stride };
	floatint r2 = { max, 2 * stride };
	floatint r3 = { max, 3 * stride };
	return (max == val.x) ? r0 :
		(max == val.y) ? r1 :
		(max == val.z) ? r2 : r3;
}

__inline__ __device__ floatint warp_reduce_findmax(floatint val_idx) {
	float val = val_idx.x;
	int idx = val_idx.y;
	float val2;
	int idx2;
	for (int offset = 16; offset > 0; offset /= 2) {
		val2 = __shfl_down(val, offset);
		idx2 = __shfl_down(idx, offset);
		val = fmaxf(val, val2);
		idx = (val == val2) ? idx2 + offset : idx;
	}
	return{ val, idx };
}
...
extern "C" __global__ void max_batch(
	...
	int * const __restrict__ ind_max
	) {
	float4 val = { correlation[threadIdx.x * 128 + blockIdx.x * (128 * 128)],
		correlation[32 * 128 + threadIdx.x * 128 + blockIdx.x * (128 * 128)],
		correlation[64 * 128 + threadIdx.x * 128 + blockIdx.x * (128 * 128)],
		correlation[96 * 128 + threadIdx.x * 128 + blockIdx.x * (128 * 128)] };
	floatint max_idx = warp_reduce_findmax(findmax4(val, 32));
	if (threadIdx.x == 0) {
		correlation_max[blockIdx.x] = max_idx.x;
		ind_max[blockIdx.x] = max_idx.y;
	}
}
...
void ZMatcher::initialization(int img_height) {
	...
	correlation_max = new float[n_ref];
	theta_ind_max = new int[n_ref];
	...
	cudaMalloc(&d_correlation_max, sizeof(float) * n_ref);
	cudaMalloc(&d_theta_ind_max, sizeof(int) * n_ref);
	cufftPlan2d(&plan_R2C, W2, H2, CUFFT_R2C);
	cufftPlanMany(&plan_C2R_batch, 2, fft_size, NULL, 1, 0, NULL, 1, 0, CUFFT_C2R, n_ref);
...
void ZMatcher::set_ref_sweep(uint16_t** pp_ref) {
	...
	free(a);
	conj_batch << <65 * n_ref, 64 >> >(d_ref_F_mag_polar_F);
}
...
int ZMatcher::match_z(uint16_t* p_moving, float * cormax) {
	//cudaSetDevice(GPU1);
	img_process(p_moving);
	cufftExecR2C(plan_R2C, d_moving_F_mag_polar, d_moving_F_mag_polar_F);
	conv_batch << <65 * n_ref, 64 >> >(d_moving_F_mag_polar_F, d_ref_F_mag_polar_F, d_correlation_F);
	cufftExecC2R(plan_C2R_batch, d_correlation_F, d_correlation);


	max_batch << <n_ref, 32 >> >(d_correlation, d_correlation_max, d_theta_ind_max);
	cudaMemcpy(correlation_max, d_correlation_max, sizeof(float) * n_ref, cudaMemcpyDeviceToHost);
	cudaMemcpy(theta_ind_max, d_theta_ind_max, sizeof(int) * n_ref, cudaMemcpyDeviceToHost);
	float max = correlation_max[n_ignore_lower];
	int idx_max = n_ignore_lower;
	for (int i = n_ignore_lower + 1; i < n_ignore_upper; i++) {
	...
	//#*theta_idx = theta_ind_max[idx_max];
	cormax[0] = max;
	return idx_max;
	...
	/*
	float * _cor = (float *)malloc(50 * 128 * 128 * sizeof(float));
	float _cormax = 0;
	int cormax_ind = 0;
	cudaMemcpy(&(_cor[0]), d_correlation, 50 * 128 * 128 * sizeof(float), cudaMemcpyDeviceToHost);
	//ippsMax_32f((Ipp32f *)_cor, 50 * 128 * 128, (Ipp32f *)(&_cormax));
	ippsMaxIndx_32f((Ipp32f *)_cor, 50 * 128 * 128, (Ipp32f *)(&_cormax), &cormax_ind);

	cormax[0] = _cormax;
	free(_cor);
	return (cormax_ind) / (W2 * H2);
	*/
}

ZMatcher::~ZMatcher() {
	delete correlation_max;
	delete theta_ind_max;
